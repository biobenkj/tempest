...
ENHANCED: Now properly handles rich metadata structures from simulated data.
"""
...
class StandardTrainer:
    ...
    def train(self, train_data: Union[np.ndarray, tuple, list], val_data: Optional[Union[np.ndarray, tuple, list]] = None, **kwargs) -> Dict[str, Any]:
        if isinstance(train_data, (list, tuple)) and len(train_data) == 2:
            ...
            # Handle list of objects (dicts, namedtuples, dataclasses)
            X_train, y_train = self._process_sequence_data(train_data)
        else:
            raise ValueError("train_data must be (X, y) tuple or list of sequence objects")

        validation_data = None
        if val_data is not None:
            if isinstance(val_data, (list, tuple)) and len(val_data) == 2:
                X_val, y_val = val_data
            elif isinstance(val_data, list) and len(val_data) > 0:
                X_val, y_val = self._process_sequence_data(val_data)
            else:
                raise ValueError("val_data must be (X, y) tuple or list of sequence objects")
    ...
    def _process_sequence_data(self, data: List) -> Tuple[np.ndarray, np.ndarray]:
        """
        Process sequence data that may be in various formats:
        - List of dicts with 'sequence' and 'labels' keys
        - List of namedtuples/dataclasses with sequence and labels attributes
        - List of objects with sequence, labels, and metadata attributes (rich format)
        """
        sequences, labels = [], []
        ...
        for item in data:
            ...
            if isinstance(label, list) and len(label) > 0 and isinstance(label[0], str):
                ...
                labels.append(label if isinstance(label, list) else [])
        ...
        logger.debug(f"Processed {len(sequences)} sequences into arrays: X={X.shape}, y={y.shape}")
        return X, y
    ...
    def evaluate(self, test_data: Union[tuple, list]) -> Dict[str, float]:
        ...
        if isinstance(test_data, (list, tuple)) and len(test_data) == 2:
            ...
            X_test, y_test = self._process_sequence_data(test_data)
        else:
            raise ValueError("test_data must be (X, y) tuple or list of sequence objects")
...
def run_training(config: TempestConfig, output_dir: Optional[Path] = None, **kwargs) -> Dict[str, Any]:
    """
    Entry point for standard training that dispatches to appropriate trainer.
    
    FIXED: No longer passes train_data as both positional and keyword argument.
    """
    # Extract data from kwargs (don't pass as positional args to avoid duplication)
    train_data = kwargs.pop('train_data', None)  # Use pop to remove from kwargs
    val_data = kwargs.pop('val_data', None)      # Use pop to remove from kwargs
    verbose = kwargs.get('verbose', False)
    mode = kwargs.get('mode', 'standard')

    # Dispatch to other trainers if needed
    if mode == 'hybrid':
        from tempest.training.hybrid_trainer import run_hybrid_training
        # Re-add train_data and val_data to kwargs for hybrid trainer
        kwargs['train_data'] = train_data
        kwargs['val_data'] = val_data
        ...
        # Re-add train_data and val_data to kwargs for ensemble trainer
        kwargs['train_data'] = train_data
        kwargs['val_data'] = val_data
        ...
        # Load data from config if not provided
        ...
        # Now call train with data as positional args (not in kwargs)
